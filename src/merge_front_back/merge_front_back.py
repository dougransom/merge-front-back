...
def merge_pdfs(front_path, back_path, output_path, verbose=False, dry_run=False):
    ...
    for i in range(total_pages):
        for source, label in [(front_pages, "front"), (back_pages, "back")]:
            if verbose:
                print(f"Adding page {i+1} from {label}")
            writer.add_page(source[i])

    if dry_run:
        print(f"[Dry Run] Would write {len(writer.pages)} pages to {output_path}")
    else:
...
def main():
    ...
    parser.add_argument("--verbose", action="store_true", help="Print page merge details")
    parser.add_argument("--dry-run", action="store_true", help="Simulate merge without writing output")

    args = parser.parse_args()
    merge_pdfs(args.front, args.back, args.merged, verbose=args.verbose, dry_run=args.dry_run)
